...
public static class WireguardApiService
{
    ...
    private static async Task<string> GetBodyAsync(string url)
    {
        ...
        if (!response.IsSuccessStatusCode)
            throw new WireguardApiException($"HTTP {(int)response.StatusCode}", body);
        return body;
    }
    ...
    private static List<WireguardServer> ParseServersList(JsonElement decoded)
    {
        ...
        foreach (var item in list.Value.EnumerateArray())
        {
            ...
            if (id is not { Length: > 0 })
                continue;
            var displayName = ReadString(item, "name", "title", "location", "country", "display_name", "server_name") ?? id;
